...
They are an industry leader in end-to-end cloud hosting solutions, as stated on their site to be “by developers for developers”. 
...
* **BUY** - 
The shop page is an online market for local produce, where we can see a map showing the producers around you, so that you can travel the least distance for your produce. 
There is also a list of items available, which will eventually be able to be sorted by distance, price and category, as well as having a search capability. There are also buttons added to edit and remove listed products. 

On the Buy Page, each item has its own profile where some of the specifics of how it was grown is listed. Review of produce functionality has added at the bottom here. 
This enables users with food restrictions & sensitivities to more carefully select their produce as well as supporting those that grows food for their dietary needs. 
Third party software cloudinary was used here to host the images, but the rest is all local assets, the products and reviews were displayed using embedded ruby to iterate over the relevant database tables. 
